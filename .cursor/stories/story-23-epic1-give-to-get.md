...
- [x] Обновить `app/services/search_service.py`:
...
### 2. Counter Increment
- [x] При каждом успешном поиске в контексте орги:
...
- [x] Добавить в `app/services/user_service.py`:
...
### 4. UX Message
- [x] Сообщение при блокировке:
  - "Лимит демо-поисков исчерпан (3/3)."
  - "Чтобы продолжить, администратор должен подтвердить твою заявку."

## Acceptance Criteria
- [x] При каждом поиске в контексте орги увеличивается счетчик `free_searches_used`
- [x] Если `status=pending` и `free_searches_used >= 3`: поиск блокируется
- [x] Бот выдает сообщение: "Лимит демо-поисков исчерпан (3/3). Чтобы продолжить, администратор должен подтвердить твою заявку."
- [x] Для `approved` пользователей лимит не действует
...
## Definition of Done
- [x] Тест: Первые 3 поиска работают для `pending` пользователя
- [x] Тест: 4-й поиск блокируется с правильным сообщением
- [x] Тест: `approved` пользователь не ограничен лимитом
- [x] Счетчик корректно инкрементируется в БД
