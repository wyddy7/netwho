-- Обновленная функция векторного поиска с поддержкой B2B (Организаций)
-- Теперь она ищет и в личных, и в общих контактах (где юзер участник).

-- Сначала дропнем старую, так как меняется сигнатура (добавляем org_id, org_name)
...
CREATE OR REPLACE FUNCTION match_contacts(
  ...
  match_threshold float DEFAULT 0.5, -- Чуть строже порог
...
BEGIN
  ...
  WHERE 
    (
      ...
      OR
      -- Общее: (Я найден в таблице участников этой орги)
